...
class MockBrowser(QtWidgets.QDialog):
    ...
    def __init__(self, url, parent=None):
        ...
        address_bar.setStyleSheet(
            "background: #1e1e1e; border: 1px solid #444; color: #aaa; border-radius: 4px; padding: 4px 8px;"
        )

        btn_open_external = QtWidgets.QPushButton("🌐 Open in browser")
        btn_open_external.setStyleSheet(
            "background: #444; border: none; padding: 4px 10px; border-radius: 4px;"
        )
        ...
        if WEB_AVAILABLE:
            ...
            fallback_message = QtWidgets.QLabel(
            ...
            QtCore.QTimer.singleShot(
                1500, lambda: [QtGui.QDesktopServices.openUrl(QtCore.QUrl(url)), self.close()]
            )
...
class TestPreviewDialog(QtWidgets.QDialog):
    ...
    def __init__(self, studio_data, jobs, logo_path=None, parent_dialog=None):
        ...
        try:
            from .studio_widget import StudioWidget
            ...
            self.preview_widget = StudioWidget(
                studio_data, MockConfigManager(jobs, logo_path, self.parent_dialog), self
            )
